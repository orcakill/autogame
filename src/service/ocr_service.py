...
class OcrService:
    ...
    @staticmethod
    def ocr_paddle(img, words: list[str], exclude_words=None, similarly=0.9):
        """
        根据图片识别文字
        ...
        try:
            ...
            # 使用单例OCR实例进行识别，禁止改为ocr函数，3.0以上版本不支持
            ...
            if ocr_result:
                for line in ocr_result:
                    ...
                    for i in range(len(rec_texts)):
                        ...
                        condition_met = (
                            any(w in text for w in words) and
                            score >= similarly and
    ...
    @staticmethod
    def ocr_paddle_list(img, words:list[str], lang='ch'):
        """
        基于PaddleOCR的指定文字识别
        ...
        if ocr_result:
            for line in ocr_result:
                ...
                for i in range(min_length):
                    ...
                    if words:
                        if any(w in text for w in words) and score >= 0.7:
